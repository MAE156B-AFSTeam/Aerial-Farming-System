...
void Movement::reportEncoders()
{
  ...
  Serial.print(" E");
  Serial.print((float)encoderE.currentPosition() / (float)stepsPerMm[1]);
  Serial.print(" Y");
  Serial.print((float)encoderY.currentPosition() / (float)stepsPerMm[2]);
  Serial.print(" Z");
  Serial.print((float)encoderZ.currentPosition() / (float)stepsPerMm[3]);
  ...
  Serial.print(" E");
  Serial.print(encoderE.currentPositionRaw());
  Serial.print(" Y");
...
void Movement::getEncoderReport()
{
  ...
  serialBuffer += " E";
  serialBuffer += (float)encoderE.currentPosition() / (float)stepsPerMm[1];
  serialBuffer += " Y";
  serialBuffer += (float)encoderY.currentPosition() / (float)stepsPerMm[2];
  serialBuffer += " Z";
  serialBuffer += (float)encoderZ.currentPosition() / (float)stepsPerMm[3];
  ...
  serialBuffer += " E";
  serialBuffer += encoderE.currentPositionRaw();
  serialBuffer += " Y";
...
void Movement::loadSettings()
{
  ...
  axisE.loadPinNumbers(E_STEP_PIN, E_DIR_PIN, E_ENABLE_PIN, X_MIN_PIN, X_MAX_PIN, 0, 0, 0);
  ...
  axisSubStep[3] = COMM_REPORT_MOVE_STATUS_IDLE;
  ...
  encoderE.loadMdlEncoderId(_MDL_X2);
  ...
  encoderE.loadPinNumbers(E_ENCDR_A, E_ENCDR_B, E_ENCDR_A_Q, E_ENCDR_B_Q);
  ...
  encoderE.loadSettings(motorConsEncoderType[1], motorConsEncoderScaling[1], motorConsEncoderInvert[1]);
  encoderY.loadSettings(motorConsEncoderType[2], motorConsEncoderScaling[2], motorConsEncoderInvert[2]);
  encoderZ.loadSettings(motorConsEncoderType[3], motorConsEncoderScaling[3], motorConsEncoderInvert[3]);
  ...
  void Movement::loadSettingsTMC2130_E()
  {
    int motorCurrentE;
    int stallSensitivityE;
    int microStepsE;

    motorCurrentE = ParameterList::getInstance()->getValue(MOVEMENT_MOTOR_CURRENT_E);
    stallSensitivityE = ParameterList::getInstance()->getValue(MOVEMENT_STALL_SENSITIVITY_E);
    microStepsE = ParameterList::getInstance()->getValue(MOVEMENT_MICROSTEPS_E);

    if (microStepsE <= 0) { microStepsE = 1; }


    axisE.loadSettingsTMC2130(motorCurrentE, stallSensitivityE, microStepsE);
  }
 ...
 */
int Movement::moveToCoords(double xDestScaled, double eDestScaled, double yDestScaled, double zDestScaled,
                                 unsigned int xMaxSpd, unsigned int eMaxSpd, unsigned int yMaxSpd, unsigned int zMaxSpd,
                                 bool xHome, bool eHome, bool yHome, bool zHome)
{
  ...
  long eDest = eDestScaled * stepsPerMm[1];
  long yDest = yDestScaled * stepsPerMm[2];
  long zDest = zDestScaled * stepsPerMm[3];
  ...
  axisE.missedStepHistory[0] = 0;
  axisE.missedStepHistory[1] = 0;
  axisE.missedStepHistory[2] = 0;
  axisE.missedStepHistory[3] = 0;
  axisE.missedStepHistory[4] = 0;
  ...
  else
  {
    ...
    if (eMaxSpd > 0 && eMaxSpd < speedMax[0])
  {
    commandSpeed[1] = eMaxSpd;
  }
  else
  {
  ...
  if (yMaxSpd > 0 && yMaxSpd < speedMax[2])
  {
    commandSpeed[2] = yMaxSpd;
  }
    else
  {
  ...
  if (zMaxSpd > 0 && zMaxSpd < speedMax[3])
  {
    commandSpeed[3] = zMaxSpd;
  }
    else
  {
    commandSpeed[3] = speedMax[3];
  }

  axisX.setMaxSpeed(commandSpeed[0]);
  axisE.setMaxSpeed(commandSpeed[1]);
  axisY.setMaxSpeed(commandSpeed[2]);
  axisZ.setMaxSpeed(commandSpeed[3]);

  // Load coordinates into axis class

  long sourcePoint[4] = {0, 0, 0, 0 }; //probably shouldnt be 0,0,0,0 maybe 0,side,diag,side
  sourcePoint[0] = CurrentState::getInstance()->getX();
  sourcePoint[1] = CurrentState::getInstance()->getE();
  sourcePoint[2] = CurrentState::getInstance()->getY();
  sourcePoint[3] = CurrentState::getInstance()->getZ();

  long currentPoint[4] = {0, 0, 0, 0 };
  currentPoint[0] = CurrentState::getInstance()->getX();
  currentPoint[1] = CurrentState::getInstance()->getE();
  currentPoint[2] = CurrentState::getInstance()->getY();
  currentPoint[3] = CurrentState::getInstance()->getZ();

  long destinationPoint[4] = {0, 0, 0, 0 };
  destinationPoint[0] = xDest;
  destinationPoint[1] = eDest;
  destinationPoint[2] = yDest;
  destinationPoint[3] = zDest;

  long homeMissedSteps[4] = { 0, 0, 0, 0 };
  ...
  if (axisE.loadCoordinates(currentPoint[1], destinationPoint[1], eHome))
  {
    Serial.print(COMM_REPORT_COORD_CHANGED_E);
    Serial.print(" E");
    Serial.print(axisE.destinationPosition() / stepsPerMm[1]);
    CurrentState::getInstance()->printQAndNewLine();
  }

  if (axisY.loadCoordinates(currentPoint[2], destinationPoint[2], yHome))
  {
    Serial.print(COMM_REPORT_COORD_CHANGED_Y);
    Serial.print(" Y");
    Serial.print(axisY.destinationPosition() / stepsPerMm[2]);
    CurrentState::getInstance()->printQAndNewLine();
  }

  if (axisZ.loadCoordinates(currentPoint[3], destinationPoint[3], zHome))
  {
    Serial.print(COMM_REPORT_COORD_CHANGED_Z);
    Serial.print(" Z");
    Serial.print(axisZ.destinationPosition() / stepsPerMm[3]);
    CurrentState::getInstance()->printQAndNewLine();
  }
  ...
  axisSubStep[3] = COMM_REPORT_MOVE_STATUS_START_MOTOR;

  reportStatus(&axisX, axisSubStep[0]);
  reportStatus(&axisE, axisSubStep[1]);
  reportStatus(&axisY, axisSubStep[2]);
  reportStatus(&axisZ, axisSubStep[3]);
  ...
  if (xHome || eHome || yHome || zHome)
  {
    if (!xHome) { axisX.deactivateAxis(); }
    if (!eHome) { axisE.deactivateAxis(); }
  ...
  while ((axisActive[0] || axisActive[1] || axisActive[2] || axisActive[3]) && !emergencyStop)
  {
    ...
    checkAxisSubStatus(&axisE, &axisSubStep[1]);
    checkAxisSubStatus(&axisY, &axisSubStep[2]);
    checkAxisSubStatus(&axisZ, &axisSubStep[3]);
    ...
    {
      axisE.checkMovement();
      checkAxisVsEncoder(&axisE, &encoderE, &motorConsMissedSteps[1], &motorLastPosition[1], &motorConsEncoderLastPosition[1], &motorConsEncoderUseForPos[1], &motorConsMissedStepsDecay[1], &motorConsEncoderEnabled[1]);
      axisE.resetStepDone();
    ...
    {
      axisY.checkMovement();
      checkAxisVsEncoder(&axisY, &encoderY, &motorConsMissedSteps[2], &motorLastPosition[2], &motorConsEncoderLastPosition[2], &motorConsEncoderUseForPos[2], &motorConsMissedStepsDecay[2], &motorConsEncoderEnabled[2]);
      axisY.resetStepDone();
    ...
    {
      axisZ.checkMovement();
      checkAxisVsEncoder(&axisZ, &encoderZ, &motorConsMissedSteps[3], &motorLastPosition[3], &motorConsEncoderLastPosition[3], &motorConsEncoderUseForPos[3], &motorConsMissedStepsDecay[3], &motorConsEncoderEnabled[3]);
      axisZ.resetStepDone();
      ...
      if (
            eHome == true &&
            (
              (homeIsUp[3] == false && currentPoint[3] <= 0) ||
              (homeIsUp[3] == true  && currentPoint[3] >= 0)
            )
        )
      {
        homeMissedSteps[3]++;
    ...
    (
      axisE.isAxisActive()
      && axisE.missedStepHistory[0] >= motorConsMissedStepsMax[1]
      //&& axisX.missedStepHistory[1] >= motorConsMissedStepsMax[0]
      //&& axisX.missedStepHistory[2] >= motorConsMissedStepsMax[0]
      //&& axisX.missedStepHistory[3] >= motorConsMissedStepsMax[0]
      //&& axisX.missedStepHistory[4] >= motorConsMissedStepsMax[0] 
    ...
    #else
    if (axisE.isAxisActive() && motorConsMissedSteps[1] >= motorConsMissedStepsMax[1])
    #endif
    {
      axisE.deactivateAxis();

      serialBuffer += "R99";
      serialBuffer += " deactivate motor E due to ";
      #if defined(FARMDUINO_EXP_V20)
      if (axisE.isDriverError())
      {
        serialBuffer += "driver error";
      }
      else
      ...
      if (eHome)
      {
        encoderE.setPosition(0);
        axisE.setCurrentPosition(0);
      }
      else
      {
        error = ERR_STALL_DETECTED_E;
      }
    }
    ...
    #else
    if (axisY.isAxisActive() && motorConsMissedSteps[2] >= motorConsMissedStepsMax[2])
    #endif
    {
    ...
    if (axisE.endStopAxisReached(false))
    {
      axisE.setCurrentPosition(0);
      encoderE.setPosition(0);
    }
    ...
    axisActive[1] = axisE.isAxisActive();
    axisActive[2] = axisY.isAxisActive();
    axisActive[3] = axisZ.isAxisActive();

    currentPoint[0] = axisX.currentPosition();
    currentPoint[1] = axisE.currentPosition();
    currentPoint[2] = axisY.currentPosition();
    currentPoint[3] = axisZ.currentPosition();

    CurrentState::getInstance()->setX(currentPoint[0]);
    CurrentState::getInstance()->setE(currentPoint[1]);
    CurrentState::getInstance()->setY(currentPoint[2]);
    CurrentState::getInstance()->setZ(currentPoint[3]);
    ...
    if (axisActive[1] == true && ((millis() >= timeStart && millis() - timeStart > timeOut[1] * 1000) || (millis() < timeStart && millis() > timeOut[1] * 1000)))
    {
      serialBuffer += COMM_REPORT_TIMEOUT_X;
      serialBuffer += "\r\n";
      serialBuffer += "R99 timeout E axis\r\n";
      error = ERR_TIMEOUT;
    }
    ...
    if (axisActive[3] == true && ((millis() >= timeStart && millis() - timeStart > timeOut[3] * 1000) || (millis() < timeStart && millis() > timeOut[3] * 1000)))
    {
    ...
    if (serialMessageDelay > 300 && serialBuffer.length() == 0 && serialBufferSending == 0)
    {
      ...
      switch(serialMessageNr)
      {
        ...
        case 2:
          ...
          if (axisE.missedStepHistory[0] > highestE) { highestE = axisE.missedStepHistory[0]; }
          if (axisE.missedStepHistory[1] > highestE) { highestE = axisE.missedStepHistory[1]; }
          if (axisE.missedStepHistory[2] > highestE) { highestE = axisE.missedStepHistory[2]; }
          if (axisE.missedStepHistory[3] > highestE) { highestE = axisE.missedStepHistory[3]; }
          if (axisE.missedStepHistory[4] > highestE) { highestE = axisE.missedStepHistory[4]; }
          ...
          serialBuffer += " U";
          serialBuffer += axisE.getNrOfSteps();
          serialBuffer += " E";
          serialBuffer += highestE;
          serialBuffer += " V";
      ...
      if (debugMessages /*&& debugInterrupt*/)
      {
				...
				Serial.print("R99");
				Serial.print(" missed step ");
				Serial.print(motorConsMissedSteps[2]);
  ...
  if (eHome && !error && !emergencyStop)
  {
    Serial.print(COMM_REPORT_HOMED_E);
    CurrentState::getInstance()->printQAndNewLine();
  }
  ...
  axisSubStep[3] = COMM_REPORT_MOVE_STATUS_STOP_MOTOR;
  ...
  CurrentState::getInstance()->setHomeMissedStepsE(homeMissedSteps[1]);
  CurrentState::getInstance()->setHomeMissedStepsY(homeMissedSteps[2]);
  CurrentState::getInstance()->setHomeMissedStepsZ(homeMissedSteps[3]);
  ...
  if (axisE.isDriverError())
  {
    Serial.print("R99 reset motor E\r\n");
    delay(100);
    axisE.initTMC2130();
    loadSettingsTMC2130_E();
    delay(100);
  ...
  Serial.print(" U");
  Serial.print(axisE.getNrOfSteps());
  Serial.print(" E");
  Serial.print(highestE);
  Serial.print(" V");
...
int Movement::calibrateAxis(int axis)
{
  ...
  case 1:
    calibAxis = &axisE;
    calibEncoder = &encoderE;
    parEndInv = MOVEMENT_INVERT_ENDPOINTS_E;
    parNbrStp = MOVEMENT_AXIS_NR_STEPS_E;
  ...
  case 3:
    ...
    invertEndStops = endStInv[3];
    missedSteps = &motorConsMissedSteps[3];
    missedStepsMax = &motorConsMissedStepsMax[3];
    lastPosition = &motorLastPosition[3];
    encoderStepDecay = &motorConsMissedStepsDecay[3];
    encoderEnabled = &motorConsEncoderEnabled[3];
    axisStatus = &axisSubStep[3];
    axisStepsPerMm = &stepsPerMm[3];
    break;
  default:
  ...
  while (!movementDone && error == 0)
  {
    ...
    if (!movementDone && ((millis() >= timeStart && millis() - timeStart > timeOut[axis] * 1000) || (millis() < timeStart && millis() > timeOut[axis] * 1000)))
    {
      ...
      case 0:
        ...
        return error;
      case 1:
        Serial.print(COMM_REPORT_TIMEOUT_E);
        CurrentState::getInstance()->printQAndNewLine();
        Serial.print("R99 timeout E axis\r\n");
        error = ERR_TIMEOUT;
        CurrentState::getInstance()->setLastError(error);
        return error;  
      case 2:
...
#endif
    {
      ...
      stepsCount++;
      if (stepsCount % (speedHome[axis] * 4) == 0) //changed to 4 from 3
      {
        // Periodically send message still active
      ...
      if (debugMessages)
      {
        if (stepsCount % (speedHome[axis] / 8) == 0 /*|| *missedSteps > 3*/)  //changed to 8 from 6
        {
          Serial.print("R99");
  ...
  *axisStatus = COMM_REPORT_MOVE_STATUS_STOP_MOTOR;
  ...
  case 0:
    ...
    break;
  case 1:
    CurrentState::getInstance()->setE(stepsCount);
    break;  
  case 2:
...
void Movement::loadMotorSettings()
{
  ...
  homeIsUp[1] = ParameterList::getInstance()->getValue(MOVEMENT_HOME_UP_E);
  homeIsUp[2] = ParameterList::getInstance()->getValue(MOVEMENT_HOME_UP_Y);
  homeIsUp[3] = ParameterList::getInstance()->getValue(MOVEMENT_HOME_UP_Z);

  speedMax[0] = ParameterList::getInstance()->getValue(MOVEMENT_MAX_SPD_X);
  speedMax[1] = ParameterList::getInstance()->getValue(MOVEMENT_MAX_SPD_E);
  speedMax[2] = ParameterList::getInstance()->getValue(MOVEMENT_MAX_SPD_Y);
  speedMax[3] = ParameterList::getInstance()->getValue(MOVEMENT_MAX_SPD_Z);

  speedMaxHome[0] = ParameterList::getInstance()->getValue(MOVEMENT_MAX_SPD_X);
  speedMaxHome[1] = ParameterList::getInstance()->getValue(MOVEMENT_MAX_SPD_E);
  speedMaxHome[2] = ParameterList::getInstance()->getValue(MOVEMENT_MAX_SPD_Y);
  speedMaxHome[3] = ParameterList::getInstance()->getValue(MOVEMENT_MAX_SPD_Z2);

  speedMin[0] = ParameterList::getInstance()->getValue(MOVEMENT_MIN_SPD_X);
  speedMin[1] = ParameterList::getInstance()->getValue(MOVEMENT_MIN_SPD_E);
  speedMin[2] = ParameterList::getInstance()->getValue(MOVEMENT_MIN_SPD_Y);
  speedMin[3] = ParameterList::getInstance()->getValue(MOVEMENT_MIN_SPD_Z);

  speedMinHome[0] = ParameterList::getInstance()->getValue(MOVEMENT_MIN_SPD_X);
  speedMinHome[1] = ParameterList::getInstance()->getValue(MOVEMENT_MIN_SPD_E);
  speedMinHome[2] = ParameterList::getInstance()->getValue(MOVEMENT_MIN_SPD_Y);
  speedMinHome[3] = ParameterList::getInstance()->getValue(MOVEMENT_MIN_SPD_Z2);

  speedHome[0] = ParameterList::getInstance()->getValue(MOVEMENT_HOME_SPEED_X);
  speedHome[1] = ParameterList::getInstance()->getValue(MOVEMENT_HOME_SPEED_E);
  speedHome[2] = ParameterList::getInstance()->getValue(MOVEMENT_HOME_SPEED_Y);
  speedHome[3] = ParameterList::getInstance()->getValue(MOVEMENT_HOME_SPEED_Z);

  stepsAcc[0] = ParameterList::getInstance()->getValue(MOVEMENT_STEPS_ACC_DEC_X);
  stepsAcc[1] = ParameterList::getInstance()->getValue(MOVEMENT_STEPS_ACC_DEC_E);
  stepsAcc[2] = ParameterList::getInstance()->getValue(MOVEMENT_STEPS_ACC_DEC_Y);
  stepsAcc[3] = ParameterList::getInstance()->getValue(MOVEMENT_STEPS_ACC_DEC_Z);

  stepsAccHome[0] = ParameterList::getInstance()->getValue(MOVEMENT_STEPS_ACC_DEC_X);
  stepsAccHome[1] = ParameterList::getInstance()->getValue(MOVEMENT_STEPS_ACC_DEC_E);
  stepsAccHome[2] = ParameterList::getInstance()->getValue(MOVEMENT_STEPS_ACC_DEC_Y);
  stepsAccHome[3] = ParameterList::getInstance()->getValue(MOVEMENT_STEPS_ACC_DEC_Z2);

  motorInv[0] = intToBool(ParameterList::getInstance()->getValue(MOVEMENT_INVERT_MOTOR_X));
  motorInv[1] = intToBool(ParameterList::getInstance()->getValue(MOVEMENT_SECONDARY_MOTOR_INVERT_X_DEFAULT));
  motorInv[2] = intToBool(ParameterList::getInstance()->getValue(MOVEMENT_INVERT_MOTOR_Y));
  motorInv[3] = intToBool(ParameterList::getInstance()->getValue(MOVEMENT_INVERT_MOTOR_Z));

  endStInv[0] = ParameterList::getInstance()->getValue(MOVEMENT_INVERT_ENDPOINTS_X);
  endStInv[1] = ParameterList::getInstance()->getValue(MOVEMENT_INVERT_ENDPOINTS_E);
  endStInv[2] = ParameterList::getInstance()->getValue(MOVEMENT_INVERT_ENDPOINTS_Y);
  endStInv[3] = ParameterList::getInstance()->getValue(MOVEMENT_INVERT_ENDPOINTS_Z);

  endStInv2[0] = ParameterList::getInstance()->getValue(MOVEMENT_INVERT_2_ENDPOINTS_X);
  endStInv2[1] = ParameterList::getInstance()->getValue(MOVEMENT_INVERT_2_ENDPOINTS_E);
  endStInv2[2] = ParameterList::getInstance()->getValue(MOVEMENT_INVERT_2_ENDPOINTS_Y);
  endStInv2[3] = ParameterList::getInstance()->getValue(MOVEMENT_INVERT_2_ENDPOINTS_Z);

  endStEnbl[0] = intToBool(ParameterList::getInstance()->getValue(MOVEMENT_ENABLE_ENDPOINTS_X));
  endStEnbl[1] = intToBool(ParameterList::getInstance()->getValue(MOVEMENT_ENABLE_ENDPOINTS_E));
  endStEnbl[2] = intToBool(ParameterList::getInstance()->getValue(MOVEMENT_ENABLE_ENDPOINTS_Y));
  endStEnbl[3] = intToBool(ParameterList::getInstance()->getValue(MOVEMENT_ENABLE_ENDPOINTS_Z));

  timeOut[0] = ParameterList::getInstance()->getValue(MOVEMENT_TIMEOUT_X);
  timeOut[1] = ParameterList::getInstance()->getValue(MOVEMENT_TIMEOUT_E);
  timeOut[2] = ParameterList::getInstance()->getValue(MOVEMENT_TIMEOUT_Y);
  timeOut[3] = ParameterList::getInstance()->getValue(MOVEMENT_TIMEOUT_Z);

  motorKeepActive[0] = ParameterList::getInstance()->getValue(MOVEMENT_KEEP_ACTIVE_X);
  motorKeepActive[1] = ParameterList::getInstance()->getValue(MOVEMENT_KEEP_ACTIVE_E);
  motorKeepActive[2] = ParameterList::getInstance()->getValue(MOVEMENT_KEEP_ACTIVE_Y);
  motorKeepActive[3] = ParameterList::getInstance()->getValue(MOVEMENT_KEEP_ACTIVE_Z);

  motorMaxSize[0] = ParameterList::getInstance()->getValue(MOVEMENT_AXIS_NR_STEPS_X);
  motorMaxSize[1] = ParameterList::getInstance()->getValue(MOVEMENT_AXIS_NR_STEPS_E);
  motorMaxSize[2] = ParameterList::getInstance()->getValue(MOVEMENT_AXIS_NR_STEPS_Y);
  motorMaxSize[3] = ParameterList::getInstance()->getValue(MOVEMENT_AXIS_NR_STEPS_Z);

  motor2Inv[0] = false;
  motor2Inv[1] = intToBool(ParameterList::getInstance()->getValue(MOVEMENT_SECONDARY_MOTOR_INVERT_X));
  motor2Inv[2] = false;
  motor2Inv[3] = false;
  
  motor2Enbl[0] = false;
  motor2Enbl[1] = intToBool(ParameterList::getInstance()->getValue(MOVEMENT_SECONDARY_MOTOR_X));
  motor2Enbl[2] = false;
  motor2Enbl[3] = false;

  motorStopAtHome[0] = intToBool(ParameterList::getInstance()->getValue(MOVEMENT_STOP_AT_HOME_X));
  motorStopAtHome[1] = intToBool(ParameterList::getInstance()->getValue(MOVEMENT_STOP_AT_HOME_E));
  motorStopAtHome[2] = intToBool(ParameterList::getInstance()->getValue(MOVEMENT_STOP_AT_HOME_Y));
  motorStopAtHome[3] = intToBool(ParameterList::getInstance()->getValue(MOVEMENT_STOP_AT_HOME_Z));

  motorStopAtMax[0] = intToBool(ParameterList::getInstance()->getValue(MOVEMENT_STOP_AT_MAX_X));
  motorStopAtMax[1] = intToBool(ParameterList::getInstance()->getValue(MOVEMENT_STOP_AT_MAX_E));
  motorStopAtMax[2] = intToBool(ParameterList::getInstance()->getValue(MOVEMENT_STOP_AT_MAX_Y));
  motorStopAtMax[3] = intToBool(ParameterList::getInstance()->getValue(MOVEMENT_STOP_AT_MAX_Z));

  stepsPerMm[0] = ParameterList::getInstance()->getValue(MOVEMENT_STEP_PER_MM_X);
  stepsPerMm[1] = ParameterList::getInstance()->getValue(MOVEMENT_STEP_PER_MM_E);
  stepsPerMm[2] = ParameterList::getInstance()->getValue(MOVEMENT_STEP_PER_MM_Y);
  stepsPerMm[3] = ParameterList::getInstance()->getValue(MOVEMENT_STEP_PER_MM_Z);
  ...
  CurrentState::getInstance()->setStepsPerMm(stepsPerMm[0], stepsPerMm[1], stepsPerMm[2], stepsPerMm[3]);
  ...
  axisE.loadMotorSettings(speedMax[1], speedMax[1], speedMin[1], speedMin[2], speedHome[1], stepsAcc[1], stepsAcc[1], timeOut[1], homeIsUp[1], motorInv[1], endStInv[1], endStInv2[1], MOVEMENT_INTERRUPT_SPEED, motor2Enbl[1], motor2Inv[1], endStEnbl[1], motorStopAtHome[1], motorMaxSize[1], motorStopAtMax[1]);
  axisY.loadMotorSettings(speedMax[2], speedMax[2], speedMin[2], speedMin[1], speedHome[2], stepsAcc[2], stepsAcc[2], timeOut[2], homeIsUp[2], motorInv[2], endStInv[2], endStInv2[2], MOVEMENT_INTERRUPT_SPEED, motor2Enbl[2], motor2Inv[2], endStEnbl[2], motorStopAtHome[2], motorMaxSize[2], motorStopAtMax[2]);
  axisZ.loadMotorSettings(speedMax[3], speedMaxHome[3], speedMin[3], speedMinHome[3], speedHome[3], stepsAcc[3], stepsAccHome[3], timeOut[3], homeIsUp[3], motorInv[3], endStInv[3], endStInv2[3], MOVEMENT_INTERRUPT_SPEED, motor2Enbl[3], motor2Inv[3], endStEnbl[3], motorStopAtHome[3], motorMaxSize[3], motorStopAtMax[3]);
...
void Movement::loadEncoderSettings()
{
  ...
  motorConsMissedStepsMax[1] = ParameterList::getInstance()->getValue(ENCODER_MISSED_STEPS_MAX_E);
  motorConsMissedStepsMax[2] = ParameterList::getInstance()->getValue(ENCODER_MISSED_STEPS_MAX_Y);
  motorConsMissedStepsMax[3] = ParameterList::getInstance()->getValue(ENCODER_MISSED_STEPS_MAX_Z);

  motorConsMissedStepsDecay[0] = ParameterList::getInstance()->getValue(ENCODER_MISSED_STEPS_DECAY_X);
  motorConsMissedStepsDecay[1] = ParameterList::getInstance()->getValue(ENCODER_MISSED_STEPS_DECAY_E);
  motorConsMissedStepsDecay[2] = ParameterList::getInstance()->getValue(ENCODER_MISSED_STEPS_DECAY_Y);
  motorConsMissedStepsDecay[3] = ParameterList::getInstance()->getValue(ENCODER_MISSED_STEPS_DECAY_Z);
  ...
  motorConsMissedStepsDecay[3] = motorConsMissedStepsDecay[3] / 100;
  ...
  motorConsMissedStepsDecay[2] = min(max(motorConsMissedStepsDecay[3], 0.01), 99);
#endif

  motorConsEncoderType[0] = ParameterList::getInstance()->getValue(ENCODER_TYPE_X);
  motorConsEncoderType[1] = ParameterList::getInstance()->getValue(ENCODER_TYPE_E);
  motorConsEncoderType[2] = ParameterList::getInstance()->getValue(ENCODER_TYPE_Y);
  motorConsEncoderType[3] = ParameterList::getInstance()->getValue(ENCODER_TYPE_Z);

  motorConsEncoderScaling[0] = ParameterList::getInstance()->getValue(ENCODER_SCALING_X);
  motorConsEncoderScaling[1] = ParameterList::getInstance()->getValue(ENCODER_SCALING_E);
  motorConsEncoderScaling[2] = ParameterList::getInstance()->getValue(ENCODER_SCALING_Y);
  motorConsEncoderScaling[3] = ParameterList::getInstance()->getValue(ENCODER_SCALING_Z);

  motorConsEncoderUseForPos[0] = ParameterList::getInstance()->getValue(ENCODER_USE_FOR_POS_X);
  motorConsEncoderUseForPos[1] = ParameterList::getInstance()->getValue(ENCODER_USE_FOR_POS_E);
  motorConsEncoderUseForPos[2] = ParameterList::getInstance()->getValue(ENCODER_USE_FOR_POS_Y);
  motorConsEncoderUseForPos[3] = ParameterList::getInstance()->getValue(ENCODER_USE_FOR_POS_Z);

  motorConsEncoderInvert[0] = ParameterList::getInstance()->getValue(ENCODER_INVERT_X);
  motorConsEncoderInvert[1] = ParameterList::getInstance()->getValue(ENCODER_INVERT_E);
  motorConsEncoderInvert[2] = ParameterList::getInstance()->getValue(ENCODER_INVERT_Y);
  motorConsEncoderInvert[3] = ParameterList::getInstance()->getValue(ENCODER_INVERT_Z);
  ...
  if (ParameterList::getInstance()->getValue(ENCODER_ENABLED_E) == 1)
  {
  ...
  {
    motorConsEncoderEnabled[3] = true;
  }
  else
  {
    motorConsEncoderEnabled[3] = false;
  }
}

unsigned long Movement::getMaxLength(unsigned long lengths[4])
{
...
void Movement::disableMotors()
{
  ...
  if (motorKeepActive[1] == 0) { axisE.disableMotor(); }
  if (motorKeepActive[2] == 0) { axisY.disableMotor(); }
  if (motorKeepActive[3] == 0) { axisZ.disableMotor(); }
...
void Movement::primeMotors()
{
  ...
  if (motorKeepActive[1] == 1) { axisE.enableMotor(); } else { axisE.disableMotor(); }
  if (motorKeepActive[2] == 1) { axisY.enableMotor(); } else { axisY.disableMotor(); }
  if (motorKeepActive[3] == 1) { axisZ.enableMotor(); } else { axisZ.disableMotor(); }
}
...
void Movement::storePosition()
{
  ...
  {
    CurrentState::getInstance()->setE(encoderE.currentPosition());
  }
  else
  {
    CurrentState::getInstance()->setE(axisE.currentPosition());
  }
...
void Movement::setPositionE(long pos)
{
  axisE.setCurrentPosition(pos);
  encoderE.setPosition(pos);
}
...
void Movement::checkEncoders()
{
  ...
  encoderX.checkEncoder(
    ...
    // A=16/PH1 B=17/PH0 AQ=31/PC6 BQ=33/PC4      need to check these register values from arduino datasheet
  encoderE.checkEncoder(
    ENC_E_A_PORT   & ENC_E_A_BYTE,
    ENC_E_B_PORT   & ENC_E_B_BYTE,
    ENC_E_A_Q_PORT & ENC_E_A_Q_BYTE,
    ENC_E_B_Q_PORT & ENC_E_B_Q_BYTE);  
