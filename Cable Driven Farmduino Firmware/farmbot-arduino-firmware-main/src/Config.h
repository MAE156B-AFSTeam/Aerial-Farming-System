...
// need to make sure all X stuff is split into X/E
#ifndef CONFIG_H_
#define CONFIG_H_
#include <math.h>
  ...
  const char COMM_REPORT_HOMED_E[4] = { 'R', '1', '2', '\0' };
  const char COMM_REPORT_HOMED_Y[4] = { 'R', '1', '3', '\0' };
  const char COMM_REPORT_HOMED_Z[4] = { 'R', '1', '4', '\0' };

  const char COMM_REPORT_COORD_CHANGED_X[4] = { 'R', '1', '5', '\0' };
  const char COMM_REPORT_COORD_CHANGED_E[4] = { 'R', '1', '6', '\0' };
  const char COMM_REPORT_COORD_CHANGED_Y[4] = { 'R', '1', '7', '\0' };
  const char COMM_REPORT_COORD_CHANGED_Z[4] = { 'R', '1', '8', '\0' };

  const char COMM_REPORT_TIMEOUT_X[4] = { 'R', '7', '1', '\0' };
  const char COMM_REPORT_TIMEOUT_E[4] = { 'R', '7', '2', '\0' };
  const char COMM_REPORT_TIMEOUT_Y[4] = { 'R', '7', '3', '\0' };
  const char COMM_REPORT_TIMEOUT_Z[4] = { 'R', '7', '4', '\0' };
  ...
  // Values used for farmplot boundaries

  const double POST_ONE_LOCATION[2]   = {0,0};
  const double POST_TWO_LOCATION[2]   = {0,1500};
  const double POST_THREE_LOCATION[2] = {1500,1500};
  const double POST_FOUR_LOCATION[2]  = {1500,0};
  const double POST_HEIGHT            = 1500;

  const double MAX_DISTANCE           = sqrt(3*pow(POST_HEIGHT,2));
  const double STEPS_PER_REV          = 200;
  const double SPOOL_DIAMETER         = 10;
  ...
  const long MOVEMENT_TIMEOUT_E_DEFAULT = 120;
  ...
  const long MOVEMENT_KEEP_ACTIVE_E_DEFAULT = 1;
  ...
  const long MOVEMENT_HOME_AT_BOOT_E_DEFAULT = 0;
  ...
  const long MOVEMENT_ENABLE_ENDPOINTS_E_DEFAULT = 0;
  ...
  const long MOVEMENT_INVERT_ENDPOINTS_E_DEFAULT = 0;
  ...
  const long MOVEMENT_HOME_UP_X_DEFAULT = 1;
  const long MOVEMENT_HOME_UP_E_DEFAULT = 1;
  const long MOVEMENT_HOME_UP_Y_DEFAULT = 1;
  const long MOVEMENT_HOME_UP_Z_DEFAULT = 1;

  const long MOVEMENT_STEP_PER_MM_X_DEFAULT = 5;
  const long MOVEMENT_STEP_PER_MM_E_DEFAULT = 5;
  const long MOVEMENT_STEP_PER_MM_Y_DEFAULT = 5;
  const long MOVEMENT_STEP_PER_MM_Z_DEFAULT = 5;

  // Number of steps used for acceleration or deceleration
  const long MOVEMENT_STEPS_ACC_DEC_X_DEFAULT = 300;
  const long MOVEMENT_STEPS_ACC_DEC_E_DEFAULT = 300;
  ...
  const long MOVEMENT_MIN_SPD_E_DEFAULT = 50;
  ...
  const long MOVEMENT_HOME_SPEED_E_DEFAULT = 50;
  ...
  const long MOVEMENT_MAX_SPD_E_DEFAULT = 400;
  ...
  const long MOVEMENT_INVERT_2_ENDPOINTS_E_DEFAULT = 0;
  ...
  const long MOVEMENT_STOP_AT_HOME_E_DEFAULT = 0;
  ...
  const long MOVEMENT_STOP_AT_MAX_E_DEFAULT = 1;
  ...
  const long MOVEMENT_MOTOR_CURRENT_E_DEFAULT = 600;
  ...
  const long MOVEMENT_STALL_SENSITIVITY_E_DEFAULT = 30;
  ...
  const long MOVEMENT_MICROSTEPS_E_DEFAULT = 0;
  ...
  const long ENCODER_ENABLED_E_DEFAULT = 0;
  ...
  const long ENCODER_TYPE_E_DEFAULT = 0;
  ...
  const long ENCODER_SCALING_E_DEFAULT = 5556;
  ...
  const long ENCODER_MISSED_STEPS_MAX_E_DEFAULT = 5;
  ...
  const long ENCODER_MISSED_STEPS_DECAY_E_DEFAULT = 5;
  ...
  const long ENCODER_USE_FOR_POS_E_DEFAULT = 0;
  ...
  const long ENCODER_INVERT_E_DEFAULT = 0;
  const long ENCODER_INVERT_Y_DEFAULT = 0;
  const long ENCODER_INVERT_Z_DEFAULT = 0;

  // Length of axis in steps. Zero means don't care
  const long MOVEMENT_AXIS_NR_STEPS_X_DEFAULT = STEPS_PER_REV*MAX_DISTANCE/(SPOOL_DIAMETER*M_PI);
  const long MOVEMENT_AXIS_NR_STEPS_E_DEFAULT = STEPS_PER_REV*MAX_DISTANCE/(SPOOL_DIAMETER*M_PI);
  const long MOVEMENT_AXIS_NR_STEPS_Y_DEFAULT = STEPS_PER_REV*MAX_DISTANCE/(SPOOL_DIAMETER*M_PI);
  const long MOVEMENT_AXIS_NR_STEPS_Z_DEFAULT = STEPS_PER_REV*MAX_DISTANCE/(SPOOL_DIAMETER*M_PI);
